...
# ************************************************************
## 2. LOGIN WINDOW CLASS (STYLED & FULL SCREEN)
# ************************************************************
class LoginWindow(QMainWindow): 
    ...
    def __init__(self):
        ...
        # --- Apply Background via QPalette (RELIABLE & SCALABLE) ---
        image_path = r"C:\Users\Phoenix Tec\Desktop\hamza_work\toss_project\resources\targetimage.png"

        self.bg_pixmap = QPixmap(image_path)  # Store as instance variable
        if self.bg_pixmap.isNull():
            print("❌ WARNING: Failed to load background image. Using solid black.")
            palette = self.palette()
            palette.setColor(QPalette.Window, QColor(0, 0, 0))
            self.setPalette(palette)
        else:
            print("✅ Background image loaded via QPixmap.")

            # Initially scale to window size
        ...
        # --- QSS for rest of styling (NO background-image property) ---
        self.setStyleSheet("""
            /* Global styles */
            QWidget {
                color: #ffffff;
    ...
    def resizeEvent(self, event):
            """Override resizeEvent to scale the background image dynamically."""
            super().resizeEvent(event)
            if hasattr(self, 'bg_pixmap') and not self.bg_pixmap.isNull():
                # Scale the pixmap to fit the current window size
                scaled_pixmap = self.bg_pixmap.scaled(
                    self.size(), 
                    Qt.KeepAspectRatioByExpanding, 
                    Qt.SmoothTransformation
...
def calculate_group_size_inches(holes):
    if len(holes) < 2:
        return 0.0
    centers = [(h[0], h[1]) for h in holes]
    max_distance_sq = 0.0
    for (c1_x, c1_y), (c2_x, c2_y) in combinations(centers, 2):
        distance_sq = (c1_x - c2_x)**2 + (c1_y - c2_y)**2
        if distance_sq > max_distance_sq:
            max_distance_sq = distance_sq
    max_distance_pixels = math.sqrt(max_distance_sq)
    group_size_inches = max_distance_pixels / PIXELS_PER_INCH
    return group_size_inches
...
class VideoThread(QThread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            current_detections = [] 
            if self.model and self.shared_data.get('index', -1) != -1:
                try:
                    results = self.model.predict(
                        frame,
                        conf=CONFIDENCE_THRESHOLD,
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QTableWidget {
                ...
                gridline-color: #3f4c5e;
                color: #ffffff;
                selection-background-color: #5d7591;
            }
            QHeaderView::section {
                background-color: #2e3b4e;
                color: #4CAF50;
                ...
                try:
                    ...
                    for line in content.splitlines():
                        if line.startswith("Shooter Name:"):
                            ...
                            try:
                                ...
                                data['group_size_inches'] = 0.0 
                        elif line.startswith("Last Image Path:"):
                            data['LastImagePath'] = line.split(': ')[1].strip()
                        elif line.strip().startswith('(') and line.strip().endswith(')'):
                            parts = line.strip('()').split(', ')
                            if len(parts) >= 2 and parts[0].isdigit() and parts[1].isdigit():
                    ...
                    if 'name' in data:
                        session_data = {
                            ...
                            "start_time": data.get('start_time', 'N/A'),
                            "holes": holes, 
                            "color": LOADED_SESSION_COLOR,
                            "group_size_inches": data.get('group_size_inches', 0.0),
                            "LastImagePath": data.get('LastImagePath', None)
                        }
                        loaded_sessions.append(session_data)
                except Exception as e:
                    print(f"Error reading or parsing session file {filename}: {e}")

        loaded_sessions.sort(key=lambda s: s.get('start_time', '0'), reverse=True)
        self.shared_data['all_sessions'].extend(loaded_sessions)
        print(f"Loaded {len(loaded_sessions)} previous sessions from disk.")
    ...
    def handle_table_click(self, row, column):
        if column == 4:
            try:
                ...
                session_to_view = None
                for session in self.shared_data['all_sessions']:
                    if (session['name'] == name and 
                        session['service_no'] == sn and 
                        session['service_year'] == sy):
                        session_to_view = session
                        break
                if session_to_view:
                    report_dialog = SessionReportDialog(session_to_view, self)
                ...
                QMessageBox.critical(self, "Error", f"Could not open report: {e}")

    def update_dashboard(self):
        self.session_table.setRowCount(0)
        active_session_index = self.shared_data.get('index', -1)
        display_list = []
        inactive_sessions = []
        active_session = None
        for i, session in enumerate(self.shared_data['all_sessions']):
            if i == active_session_index:
                active_session = session
            else:
                inactive_sessions.append(session)
        if active_session:
            display_list.append(active_session)
        inactive_sessions.sort(key=lambda s: s.get('start_time', '0'), reverse=True)
        display_list.extend(inactive_sessions)

        self.session_table.setRowCount(len(display_list))
        for row, session in enumerate(display_list):
    ...
    def start_new_session(self):
        if self.shared_data.get('index', -1) != -1:
            QMessageBox.warning(self, "Action Denied", "Please stop the currently running session first.")
            return
        name = self.name_input.text().strip()
        service_no = self.service_input.text().strip()
        service_year = self.service_year_input.text().strip()
        start_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        if not name or not service_no or not service_year:
            QMessageBox.warning(self, "Input Error", "Please fill in all session details (Name, SN, SY).")
            return
        ...
        new_session = {
            ...
            "holes": [],
            "color": COLOR_PALETTE[len(self.shared_data['all_sessions']) % len(COLOR_PALETTE)],
            "group_size_inches": 0.0,
            "LastImagePath": None
        }

        self.shared_data['all_sessions'].append(new_session)
        self.shared_data['index'] = len(self.shared_data['all_sessions']) - 1
        self.shared_data['session_details'] = {
            'studentName': name, 
            'serviceNo': service_no, 
            'serviceYear': service_year
        ...
        self.service_year_input.clear()
        self.status_label.setText(f"Status: Session '{name}' started at {start_time}.")
        self.stacked_widget.setCurrentIndex(1)
        self.update_dashboard()
    ...
    def stop_current_session(self):
        current_index = self.shared_data.get('index', -1)
        if current_index == -1:
            QMessageBox.information(self, "Session Status", "No active session is running.")
            return
        session = self.shared_data['all_sessions'][current_index]

        session_holes = session["holes"]
        center_coords = [h[:2] for h in session_holes if len(h) >= 2] 
        group_size = calculate_group_size_inches(center_coords)
        session['group_size_inches'] = group_size

        image_name = f"session_{session['start_time']}.jpg"
        image_path = os.path.join(IMAGE_SAVE_DIR, image_name)
        if self.thread.last_frame is not None:
            cv2.imwrite(image_path, self.thread.last_frame)
            session['LastImagePath'] = image_path
            print(f"Session image saved to: {image_path}")
        else:
            session['LastImagePath'] = None
            print("WARNING: Could not save session image (frame was None).")

        file_name = f"{session['service_no']}_{session['service_year']}_{session['name']}_{session['start_time']}.txt"
        ...
        with open(file_path, 'w') as f:
            ...
            f.write(f"Start Time: {session['start_time']}\n")
            f.write(f"Total Hits: {len(session['holes'])}\n")
            f.write(f"Extreme Spread (Group Size): {group_size:.2f} inches\n")
            f.write(f"Last Image Path: {session['LastImagePath'] if session['LastImagePath'] else 'N/A'}\n")
            f.write("\n--- Hole Coordinates (cx, cy) ---\n")
            for hole in session_holes:
                f.write(f"({hole[0]}, {hole[1]})\n")

        session["color"] = LOADED_SESSION_COLOR
        self.shared_data['index'] = -1
        self.status_label.setText(f"Status: Session '{session['name']}' stopped. Hits: {len(session['holes'])}, Group Size: {group_size:.2f} inches.")
        self.update_dashboard()

        report_dialog = SessionReportDialog(session, self)
        report_dialog.exec_()
        self.stacked_widget.setCurrentIndex(2)
    ...
    def delete_all_old_sessions(self):
        ...
        reply = QMessageBox.question(
            self, 
            "Confirm Deletion",
            "Are you sure you want to delete ALL OLD sessions? This cannot be undone.",
            QMessageBox.Yes | QMessageBox.No, 
            QMessageBox.No
        )
        if reply == QMessageBox.Yes:
            ...
            for filename in os.listdir(SAVE_DIR):
                if filename.endswith(".txt"):
                    ...
                    try:
                        ...
                        print(f"Error deleting file {file_path}: {e}")
            self.shared_data['all_sessions'] = []
            self.load_all_sessions() 
            QMessageBox.information(self, "Success", f"Successfully deleted {deleted_count} old session files.")
            self.update_dashboard()
    ...
    def update_hit_count(self, count):
        if self.shared_data.get('index', -1) != -1:
            self.session_table.setItem(0, 3, QTableWidgetItem(str(count)))
